...
export const redis = createInstance();
export const redisPubSub = createInstance();
...
function createInstance() {
    ...
    instance
        .on("error", (error: any) => logger.warn("Redis Client Error", {error, database, host, port}))
        .on("connect", () => logger.info("Connected to Redis DB"))
        .on("ready", () => logger.info("Client ready to use Redis DB"))
        .on("end", () => logger.warn("Client disconnected from Redis DB"));
    return instance;
}
