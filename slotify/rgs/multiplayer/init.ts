...
export async function init(room: Room): Promise<Draw> {
    const time = Date.now();
    const {state, nextTickTime} = await initRequest(room, {config: room.config, time});
    const drawId = v4();
    return await Draw.create({roomId: room.roomId, state, nextTickTime, drawId, finished: false, tickId: 0}).save();
}

async function initRequest(room: Room, request: {time: number; config: any}): Promise<{state?: any; nextTickTime: number; broadcast?: any}> {
