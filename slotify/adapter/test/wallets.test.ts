...
const getById = async (id: string) => {
    const query = gql`
        query ($id: JSON) {
            wallets(filter: [{field: "id", type: EQUAL, value: $id}]) {
                items {
                    adapter
                    config
                    id
...
describe("wallets", () => {
    test("get", async () => {
        ...
        expect(res.body.data.wallets.items).toContainEqual({id: "demo", adapter: "standard", config: {"secretKey": "demo-secret", "url": "http://demo-casino"}});
    });

    test("add", async () => {
        expect((await add("test-id-add", "test-adapter", {test: "abc"})).data.addWallet).toEqual(expect.any(String));
    ...
    test("edit", async () => {
        ...
        const query = gql`
            mutation ($data: WalletInput!) {
                editWallet(id: "test-id-edit", data: $data)
            }
        `;
        ...
        expect(res.body.data.editWallet).toEqual(true);
    ...
    test("edit - duplicated", async () => {
        ...
        const query = gql`
            mutation ($data: WalletInput!) {
                editWallet(id: "test-id-edit-duplicated-2", data: $data)
            }
        `;
    ...
    test("delete", async () => {
        ...
        const query = gql`
            mutation {
                deleteWallet(id: "test-id-delete")
            }
        `;
        const res = await graphQlRequest(api, await accountToken(api), query).expect(200);
        expect(res.body.data.deleteWallet).toEqual(true);
