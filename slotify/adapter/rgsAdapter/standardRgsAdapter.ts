...
const adapter: IRgsAdapter = {
    init: function init(rgs, api, basePath, config: IConfig) {
        ...
        api.put(
            ...
            async (req, res) => {
                const {rgsTransactionId, playerId, type, amount, jackpotAmount, provider, game, variant, roundId, category, name, campaignType, campaignId, roundFinished, channel, winRatio, ip, auto = false, regulatory} = req.body;
